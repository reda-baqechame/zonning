"use client";

import { useState } from "react";
import { useLocale, useTranslations } from "next-intl";
import { COVERAGE_CITIES } from "@/lib/datasets/registry";
import SiteIntelligencePanel, { intelAccessForPlan } from "@/components/SiteIntelligencePanel";
import type { PropertyIntelligence } from "@/lib/intelligence";
import type { ZoningExpertAnalysis } from "@/lib/zoning/expert-analysis";
import ZoningExpertPanel from "@/components/ZoningExpertPanel";
import { Button, Input, FieldLabel, Select } from "@/components/ui";

export default function IntelligenceExplorerClient({ plan = "FREE" }: { plan?: string | null }) {
  const t = useTranslations("intelExplorer");
  const locale = useLocale();
  const [address, setAddress] = useState("");
  const [city, setCity] = useState("Montréal");
  const [borough, setBorough] = useState("");
  const [desiredUse, setDesiredUse] = useState("");
  const [proposedFloors, setProposedFloors] = useState("");
  const [proposedUnits, setProposedUnits] = useState("");
  const [intel, setIntel] = useState<PropertyIntelligence | null>(null);
  const [zoningAnalysis, setZoningAnalysis] = useState<ZoningExpertAnalysis | null>(null);
  const [emptyMessage, setEmptyMessage] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  async function lookup() {
    if (!address.trim()) return;
    setLoading(true);
    setError(null);
    setEmptyMessage(null);
    try {
      const params = new URLSearchParams({ address: address.trim() });
      if (city) params.set("city", city);
      if (borough) params.set("borough", borough);
      if (desiredUse.trim()) params.set("desiredUse", desiredUse.trim());
      if (proposedFloors) params.set("proposedFloors", proposedFloors);
      if (proposedUnits) params.set("proposedUnits", proposedUnits);
      const res = await fetch(`/api/intelligence?${params}`);
      const data = await res.json();
      if (!res.ok) throw new Error(t("lookupFailed"));
      setZoningAnalysis(data.zoningAnalysis ?? null);
      if (data.hasData === false) {
        setIntel(null);
        setEmptyMessage(t("noMatch"));
      } else {
        setIntel(data.intelligence ?? data);
      }
    } catch (e) {
      setError(e instanceof Error ? e.message : t("lookupFailed"));
      setIntel(null);
      setZoningAnalysis(null);
    } finally {
      setLoading(false);
    }
  }

  const access = intelAccessForPlan(plan);

  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      <h1 className="text-3xl font-bold text-ink">{t("title")}</h1>
      <p className="mt-2 text-muted">{t("subtitle")}</p>

      <div className="mt-8 space-y-4 rounded-lg border border-line bg-surface p-6">
        <div>
          <FieldLabel>{t("address")}</FieldLabel>
          <Input
            value={address}
            onChange={(e) => setAddress(e.target.value)}
            placeholder={t("addressPlaceholder")}
          />
        </div>
        <div className="grid gap-4 sm:grid-cols-3">
          <div>
            <FieldLabel htmlFor="desired-use">{t("desiredUse")}</FieldLabel>
            <Input
              id="desired-use"
              value={desiredUse}
              onChange={(e) => setDesiredUse(e.target.value)}
              placeholder={t("desiredUsePlaceholder")}
            />
          </div>
          <div>
            <FieldLabel htmlFor="proposed-floors">{t("proposedFloors")}</FieldLabel>
            <Input
              id="proposed-floors"
              type="number"
              min="1"
              max="500"
              value={proposedFloors}
              onChange={(e) => setProposedFloors(e.target.value)}
            />
          </div>
          <div>
            <FieldLabel htmlFor="proposed-units">{t("proposedUnits")}</FieldLabel>
            <Input
              id="proposed-units"
              type="number"
              min="1"
              max="500"
              value={proposedUnits}
              onChange={(e) => setProposedUnits(e.target.value)}
            />
          </div>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <FieldLabel>{t("city")}</FieldLabel>
            <Select value={city} onChange={(e) => setCity(e.target.value)}>
              {COVERAGE_CITIES.map((c) => (
                <option key={c} value={c}>
                  {c}
                </option>
              ))}
            </Select>
          </div>
          <div>
            <FieldLabel>{t("borough")}</FieldLabel>
            <Input
              value={borough}
              onChange={(e) => setBorough(e.target.value)}
              placeholder={t("boroughOptional")}
            />
          </div>
        </div>
        <Button onClick={lookup} disabled={loading}>
          {loading ? t("loading") : t("search")}
        </Button>
        {error && (
          <p role="alert" className="border-l-4 border-danger bg-danger-soft p-3 text-sm text-danger-ink">
            {error}
          </p>
        )}
        {emptyMessage && (
          <p className="border-l-4 border-warning bg-warning-soft p-3 text-sm text-warning-ink">
            {emptyMessage}
          </p>
        )}
      </div>

      {intel && (
        <div className="mt-8">
          <SiteIntelligencePanel intel={intel} locale={locale} access={access} />
        </div>
      )}
      {zoningAnalysis ? (
        <div className="mt-8 rounded-lg border border-line bg-surface p-5">
          <ZoningExpertPanel analysis={zoningAnalysis} />
        </div>
      ) : null}
    </div>
  );
}
